...
export default function PetListByCategory() {
  ...
  const GetPets = async () => {
    try {
      const snapshot = await getDocs(collection(db, 'Pet'));
      const fetchedPets = snapshot.docs.map((doc) => ({ id: doc.id, ...doc.data() }));

      // Organizing pets by category
      const categorizedPets = fetchedPets.reduce((acc, pet) => {
        const category = pet.category || "Others"; // Default category if missing
        if (!acc[category]) {
          acc[category] = [];
        }
        acc[category].push({
          ...
          name: pet.name,
          breed: pet.breed,
          age: pet.age + " YRS", // Formatting age
          imageUrl: pet.image || pet.imageUrl, // Handling inconsistent image field
        });
        return acc;
      }, {});
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.categoryContainer}>
        <TouchableOpacity onPress={() => handleCategoryPress('dog')} style={[styles.iconWrapper, styles.dogsBg]}>
          <Image source={require('./../../assets/images/dog.png')} style={styles.icon} />
          <Text style={styles.categoryText}>Dogs</Text>
        </TouchableOpacity>

        <TouchableOpacity onPress={() => handleCategoryPress('cat')} style={[styles.iconWrapper, styles.catsBg]}>
          <Image source={require('./../../assets/images/cat.png')} style={styles.icon} />
          <Text style={styles.categoryText}>Cats</Text>
        </TouchableOpacity>

        <TouchableOpacity onPress={() => handleCategoryPress('bird')} style={[styles.iconWrapper, styles.birdsBg]}>
          <Image source={require('./../../assets/images/bird.png')} style={styles.icon} />
      ...
      {selectedCategory && pets[selectedCategory] && (
        <FlatList
          data={pets[selectedCategory]}
